...
export class FiltroClientePipe implements PipeTransform {
  ...
  transform(clientes: any[], texto: string, estado: string, genero: string): any[] {
    ...
    return clientes.filter(item => {
      const matchesTexto = item.name.toLowerCase().includes(texto) || item.lastName.toLowerCase().includes(texto) || item.documentIdentificationNumber.toString().includes(texto);
